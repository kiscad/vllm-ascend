...
class GetMaskedInputAndMask
{
  public:
    ...
    __aicore__ inline void Init(__gm__ scalar_t* input,
                                ...
                                const int64_t    size)
    {
        ...
        size_                  = ((size + 31) / 32) * 32;
        added_offset_ = added_vocab_start_index - (org_vocab_end_index - org_vocab_start_index) -
        ...
        pipe.InitBuffer(inputFloat_buf,
                        size_ * sizeof(float));  // Also used for half intermediate in casting
  ...
  private:
    ...
    __aicore__ inline void CompareWithValue(AscendC::LocalTensor<int8_t>&      result,
    ...
    __aicore__ inline void ComputeRangeMask(AscendC::LocalTensor<int8_t>&      range_mask,
                                            const AscendC::LocalTensor<float>& input,
    ...
    __aicore__ inline void Compute()
    {
        ...
        AscendC::Cast(
                addedVocabMask_fp32, addedVocabMask_fp16, AscendC::RoundMode::CAST_NONE, size_);
...
get_masked_input_and_mask_kernel(__gm__ int32_t* input,
...
namespace vllm_ascend
{
    ...
    void get_masked_input_and_mask_impl(void*          stream,
                                        void*          input,
                                        void*          masked_input,
                                        void*          mask_out,
                                        const int64_t  org_vocab_start_index,
