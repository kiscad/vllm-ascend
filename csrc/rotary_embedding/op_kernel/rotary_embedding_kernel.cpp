...
// retrieve this size from runtime for more Soc support
  ...
  public:
    ...
    // Allocate buffers for input and output queue and the temp buffer used during kernel compute
    // process, this init process happens only in the kernel compute on a single vector core.
    __aicore__ inline void init(__gm__ int64_t*  positions,
                                __gm__ void*     queryDst,
                                __gm__ void*     keyDst,
                                __gm__ scalar_t* query,
                                __gm__ scalar_t* key,
                                __gm__ scalar_t* cosSinCache,
                                const int        rotDim,
                                ...
                                AscendC::TPipe*  pipe)
    {
        pipe_   = pipe;
        rotDim_ = rotDim;
        // query stride and key stride is used to handle the strided tensor which is not contiguous
        // on num_tokens dim
        ...
        pipe_->InitBuffer(
                inQueSinCos_, 1 /* buffer_num */, rotDim_ * sizeof(scalar_t) /* buffer_size */);
        ...
        // 2 temporary buffer, 2 input buffer, 1 cos buffer, 1 sin buffer, 2 scale buffer
        // (headSize), 2 zp buffer(headSize int8), 1 dst_temp buffer(headSize, int32)
    ...
    __aicore__ inline void update_mem_offset(__gm__ int64_t*  positions,
                                             __gm__ void*     queryDst,
                                             __gm__ void*     keyDst,
                                             __gm__ scalar_t* query,
                                             __gm__ scalar_t* key,
                                             ...
                                             const int64_t    idx)
    {
        ...
        queryDst_.SetGlobalBuffer(reinterpret_cast<__gm__ dst_t*>(queryDst) + dstQueryStride * idx,
                                  headSize * numHeads_);
        keyDst_.SetGlobalBuffer(reinterpret_cast<__gm__ dst_t*>(keyDst) + dstKeyStride * idx,
                                headSize * numKvHeads_);
    }

    // compute per head for neox on bf16
    template <typename acc_t_,
              typename std::enable_if<!std::is_same_v<acc_t_, scalar_t>, void>::type* = nullptr>
    __aicore__ inline void neox_compute(local_scalar_t               src,
                                        local_dst_t                  dst,
    ...
    __aicore__ inline void compute_qk(AscendC::GlobalTensor<scalar_t> srcG,
                                      ...
                                      local_acc_t                     localCos,
                                      local_acc_t                     localSin,
                                      ...
                                      int                             headNumPerLoad)
    {
        for (int loopNum = 0; loopNum < loopCnt; ++loopNum)
        {
            ...
            {
                neox_compute(srcDeque[i * headSize_],
                             dst[i * headSize_],
                             localSin,
                             localCos,
                             upcastInputBuffer,
...
// Note: Need to use macro to instaniate all the target functions here, for the current build system
// dose not support template call in cpp We use C style symbol here for kernel compilation, cpp
// style kernel entry may lead to compilation failure
#define ROPE_CUSTOM_KERNEL_TYPE_DECLARE(TYPE, NEOX)                                                \
    ...
    {                                                                                              \
        ...
        {                                                                                          \
            op.update_mem_offset(positions,                                                        \
                                 queryDst,                                                         \
                                 keyDst,                                                           \
                                 query,                                                            \
                                 key,                                                              \
...
#define ROTARY_EMBEDDING_KERNEL_CALL(TYPE)                                                         \
    if (isNeox)                                                                                    \
        rope_custom_true_##TYPE<<<blockDim, nullptr, stream>>>(                                    \
                ...
                reinterpret_cast<TYPE*>(cosSinCache),                                              \
                rotDim,                                                                            \
                queryStride,                                                                       \
                keyStride,                                                                         \
                dstQueryStride,                                                                    \
