...
class SGMVExpand
{
  public:
    ...
    static constexpr uint64_t SUPPORTED_RANKS[] = {
            LORA_RANK_8, LORA_RANK_16, LORA_RANK_32, LORA_RANK_64};
    static constexpr int32_t BUFFER_NUM = 2;

    // The vector unit reads 8 blocks (32 bytes each and 256 bytes in total) of contiguous data each
    // time.
    ...
    // Refer to numOutputElementsPerInputTile_ initialization for the constraints on the following
    // constants.
    static constexpr int32_t W_IN_TILE_NUM_ELEMENTS  = 8192;
    static constexpr int32_t Y_OUT_TILE_NUM_ELEMENTS = 4096;
    static constexpr int32_t BLOCK_REDUCE_NUM_REPEATS =
            W_IN_TILE_NUM_ELEMENTS / NUM_ELEMENTS_PER_REPEAT;
  ...
  public:
    ...
    __aicore__ inline void Init(__gm__ void* x,
                                __gm__ void* weight,
                                __gm__ void* loraIndices,
                                ...
                                uint32_t     outputFullDim)
    {
        ...
        // Therefore, the following variable would determine how many output elements are calculated
        // in each iteration.
        numOutputElementsPerInputTile_ =
                BLOCK_REDUCE_NUM_REPEATS * (NUM_ELEMENTS_PER_REPEAT / maxLoRARank_);
  ...
  private:
    ...
    __aicore__ inline void ComputeLastIteration()
    {
        ...
        if (remainingWForLastRepeat != 0)
        {
            ...
            Compute(lastComputeOutputElement,
                    lastRepeatCount,
                    pairReduceRepeat16,
                    pairReduceRepeat32);
        }
    ...
    __aicore__ inline void CopyInW(int32_t progress, int32_t numElements = W_IN_TILE_NUM_ELEMENTS)
    {
        ...
        DataCopy(wLocal,
                 wGm_[reqLoRAWeightOffset_ + progress * W_IN_TILE_NUM_ELEMENTS],
                 numElements);
                                   ...
                                   int32_t pairReduceRepeat32     = PAIR_REDUCE_NUM_REPEATS_32)
    {
        ...
        {
            BlockReduceSum(yLocal[progress],
                           wTmpTensor,
                           blockReduceRepeatCount,
                           MASK_COUNT,
  ...
  private:
    ...
    // When the repeat stride is 0, the vector unit repeatedly reads and computes the first 8
    // consecutive blocks. For xDup we repeatedly use it, so we set src0RepStride = 0
    AscendC::BinaryRepeatParams dotProductParams_ = {1, 1, 1, 8, 0, 8};
};

#define SGMV_EXPAND_TYPE_DECLARE(TYPE)                                                             \
    extern "C" __global__ __aicore__ void sgmv_expand_##TYPE(__gm__ void* x,                       \
                                                             __gm__ void* weight,                  \
                                                             __gm__ void* loraIndices,             \
                                                             uint32_t     loraIndicesSize,         \
                                                             __gm__ void* seqLen,                  \
...
namespace vllm_ascend
{
                                 ...
                                 uint32_t   outputFullDim)
    {
        ...
        {
            sgmv_expand_half<<<blockDim, nullptr, stream>>>(x,
                                                            weight,
                                                            loraIndices,
                                                            loraIndicesSize,
                                                            seqLen,
        ...
        {
#if (__CCE_AICORE__ >= 220)
            sgmv_expand_bfloat16_t<<<blockDim, nullptr, stream>>>(x,
                                                                  weight,
                                                                  loraIndices,
                                                                  loraIndicesSize,
                                                                  seqLen,
